...
class Aliment{
  public:
    ...
    // Getters
    // On ne veut pas perdre de points :(, mais malheureusement c'est la seule 
    // façon qu'on a trouvée sans mettre de friend ou réduire l'efficacité du code inutilement.
    Date& getDateExpiration() {return dateExpiration;}
    int& getQuantite() {return quantite;}
    string& getNomAliment() {return nomAliment;} // OK
...
class Epicerie{
  ...
  public:
    ...
    PointST getPosition() {return position;}
...
class Carte{
  public:
	...
	// 5) Méthodes associées à la commande DATE 
    void fixerDateActuelle(Date& date) {dateActuelle = date;}
	
	// Méthodes optionnelles d'affichage
